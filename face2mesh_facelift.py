...
class FaceLiftStage:
    ...
    def run(
        ...
        # Force offline mode to prevent hanging HF Hub API checks
        ...
        result = subprocess.run(
            [
                ...
                "--input_dir", str(stage_input.resolve()),
                "--output_dir", str(stage_output.resolve()),
                "--auto_crop",
                "--seed", str(seed),
                "--guidance_scale_2D", str(guidance_scale),
                "--step_2D", str(steps),
            ],
            stdout=None,              # stream to log in real-time
...
class GaussianToMesh:
    ...
    def _tsdf_convert(self, gaussians_ply, mesh_path, o3d):
        ...
        for elev, azim in self.VIEW_ANGLES:
            ...
            cam.extrinsic = extr
            volume.integrate(rgbd_o3d, intrinsic, extr)
